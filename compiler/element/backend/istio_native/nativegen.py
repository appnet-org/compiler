...
from compiler.element.backend.istio_native.appnettype import DEFAULT_DECORATOR
from compiler.element.backend.istio_native.types import *
from compiler.element.backend.istio_native.types import NativeVariable
...
class NativeGenerator(Visitor):
    ...
    def visitState(self, node: State, ctx: NativeContext) -> None:
        ...
        for (identifier, type, cons, comb, per) in node.state:
            ...
            native_var, decl = ctx.declareNativeVar(
    ...
    def visitForeach(self, node: Foreach, ctx: NativeContext):
        ...
        iter_var_native, _decl = ctx.declareNativeVar(
    ...
    def generateOptionMatch(
        ...
        bind_native_var, decl = ctx.declareNativeVar(
    ...
    def visitAssign(self, node: Assign, ctx: NativeContext) -> None:
        ...
        if isinstance(node.left, Identifier):
            ...
            if ctx.find_appnet_var(lhs_name) is None:
                ...
                lhs_native, decl = ctx.declareNativeVar(
            ...
            first_var_native, first_decl = ctx.declareNativeVar(
                first_name, rhs_appnet_type.first.to_native()
            )
            second_var_native, second_decl = ctx.declareNativeVar(
    ...
    def visitPair(
        self, node: Pair, ctx: NativeContext
    ) -> Tuple[AppNetType, NativeVariable]:
        ...
        native_pair, decl = ctx.declareNativeVar(
    ...
    def visitExpr(
        self, node: Expr, ctx: NativeContext
    ) -> Tuple[AppNetType, NativeVariable]:
    ...
    def visitFuncCall(
        self, node: FuncCall, ctx: NativeContext
...
class GetMapStrongConsistency(AppNetBuiltinFuncProto):
    ...
    def gen_code(
        self, ctx: NativeContext, map: NativeVariable, key: NativeVariable
    ) -> NativeVariable:
...
class TimeDiff(AppNetBuiltinFuncProto):
    ...
    def gen_code(
        self, ctx: NativeContext, end: NativeVariable, start: NativeVariable
    ) -> NativeVariable:
...
class Min(AppNetBuiltinFuncProto):
    ...
    def gen_code(
        self, ctx: NativeContext, a: NativeVariable, b: NativeVariable
    ) -> NativeVariable:
...
class GetBackEnds(AppNetBuiltinFuncProto):
    ...
    def gen_code(
        self, ctx: NativeContext, backend_name: NativeVariable
    ) -> NativeVariable:
        ...
        url_native, decl = ctx.declareNativeVar(
            ctx.new_temporary_name(), NativeString()
        )
        ctx.push_code(decl)
        ctx.push_code(
...
class RandomChoices(AppNetBuiltinFuncProto):
    ...
    def gen_code(
        self, ctx: NativeContext, vec: NativeVariable, num: NativeVariable
    ) -> NativeVariable:
...
class GetLoad(AppNetBuiltinFuncProto):
    ...
    def gen_code(
        self, ctx: NativeContext, backend_id: NativeVariable
    ) -> NativeVariable:
...
class SetMetadata(AppNetBuiltinFuncProto):
    ...
    def gen_code(
        self, ctx: NativeContext, key: NativeVariable, value: NativeVariable
    ) -> None:
