...
from experiments.utils_bookinfo import *
...
yml_header = {
    "envoy": """app_name: "bookinfo"
app_structure:
-   "productpage->reviews"
-   "productpage->details"
-   "reviews->ratings"
-   "reviews->mongodb-reviews"
-   "details->mongodb-details"
-   "ratings->mongodb-ratings"
""",
}
...
def run_trial(curr_trial_num) -> List[Element]:
    ...
    for mode in results.keys():
        ...
        spec_path = (
            "generated_bookinfo/randomly_generated_spec_weak.yml"
            if "data_weak" in mode
            else "generated_bookinfo/randomly_generated_spec_strong.yml"
        )
        ...
        kapply_and_sync(os.path.join(graph_base_dir, "generated", "bookinfo_deploy"))
        ...
        if mode != "opt_no":
            EVAL_LOG.info(f"[{mode}] Bypassing productpage...")
            for node in node_pool:
                bypass_sidecar(node, "productpage", "8080", "S")
...
if __name__ == "__main__":
    ...
    #     EXP_DIR, "elements/bookinfo_elements"
    # )
